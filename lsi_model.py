...
class LSIModel:
    ...
    def __init__(self, config, tokenizer, build=False):
        self.config = config
    ...
    def get_topic_words_matrix(self):
        if not os.path.isfile(self.topic_words_matrix_file):
            ...
            topic_word_matrix = np.zeros((self.config.num_topics, len(self.tokenizer)))  # K x V (num_topics x vocab_size)
            try:
            ...
            for i in range(len(dictionary)):
                j = self.tokenizer.convert_tokens_to_ids(lsi_model.id2word[i])
